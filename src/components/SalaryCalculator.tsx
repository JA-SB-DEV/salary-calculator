...
const SalaryCalculator: React.FC<SalaryCalculatorProps> = ({
  ...
  useEffect(() => {
    const puntosTotales =
      puntosFormacion[formacion] +
      puntosCategoria[categoria] +
      experiencia * 2 +
      publicaciones * 3;
    ...
    <div className="container">
      {presupuesto === null ? (
        <>
          ...
          />
          <button onClick={handleSetPresupuesto}>
        ...
        <>
          ...
          <div className={`presupuesto ${getPresupuestoClass()}`}>
            <h3>
              Presupuesto disponible: ${presupuestoRestante!.toLocaleString()}
            </h3>
          </div>
          {mensaje && (
            <div className={`mensaje ${notificationClass}`}>{mensaje}</div>
          ...
          <div className="form-container">
            <div className="form-section">
              ...
              <select
                id="tipoDocente"
                value={tipoDocente}
                onChange={(e) => setTipoDocente(e.target.value as TipoDocente)}
              >
              ...
              <select
                id="formacion"
                value={formacion}
                onChange={(e) => setFormacion(e.target.value as Formacion)}
              >
