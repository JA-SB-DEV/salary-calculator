...
const App: React.FC<AppProps> = ({ presupuesto, setPresupuesto, presupuestoRestante, setPresupuestoRestante }) => {
  const [teachers, setTeachers] = useState<Teacher[]>([]);
  const [id, setId] = useState('');
  const [nombre, setNombre] = useState('');
  const [tipoOcasional, setTipoOcasional] = useState('');
  const [cualificacion, setCualificacion] = useState<'Especialización' | 'Maestría' | 'Doctorado' | 'Ninguna'>('Ninguna');
  const [grupoColciencias, setGrupoColciencias] = useState<'Grupo A1' | 'Grupo A' | 'Grupo B' | 'Grupo C' | 'Grupos Reconocidos' | 'Semillero' | 'Ninguno'>('Ninguno');
  const [salarioFinal, setSalarioFinal] = useState(0);
  const [mensaje, setMensaje] = useState('');
  const [mostrarValoresTipo, setMostrarValoresTipo] = useState(false);
  const [mostrarValoresCualificacion, setMostrarValoresCualificacion] = useState(false);
  ...
  useEffect(() => {
    ...
    const total = savedTeachers.reduce((acc, teacher) => acc + calculateSalary(teacher), 0);
    setTotalSalarios(total);
  }, []);

  useEffect(() => {
    const tempTeacher = new Teacher(id, nombre, 'ocasional', tipoOcasional, cualificacion, grupoColciencias);
    setSalarioFinal(calculateSalary(tempTeacher));
  }, [id, nombre, tipoOcasional, cualificacion, grupoColciencias]);

  useEffect(() => {
    if (presupuesto !== null) {
      const total = teachers.reduce((acc, teacher) => acc + calculateSalary(teacher), 0);
      setPresupuestoRestante(presupuesto - total);
    }
  }, [presupuesto, teachers]);

  const handleAddTeacher = () => {
    const newTeacher = new Teacher(id, nombre, 'ocasional', tipoOcasional, cualificacion, grupoColciencias);
  ...
  const handleDeleteTeacher = () => {
    if (idBorrar) {
      ...
      const updatedTeachers = getTeachers();
      setTeachers(updatedTeachers);
      const total = updatedTeachers.reduce((acc, teacher) => acc + calculateSalary(teacher), 0);
      setTotalSalarios(total);
      setPresupuestoRestante(presupuesto! - total);
    ...
    <div className="container">
      <div className="main-content">
        <div className="header">
          ...
          <button className="explanation-button" onClick={toggleExplanation}>?</button>
        </div>
        ...
        {showExplanation && (
          <div className="explanation">
            ...
            <p>El salario base se multiplica por un factor dependiendo del tipo de docente ocasional. 
              Luego, se suman porcentajes adicionales según la cualificación y el grupo Colciencias del docente.</p>
          </div>
        )}
        <div className="form-container">
          <div className="form-section">
            ...
            <div className="tipo-ocasional-container">
              <select value={tipoOcasional} onChange={(e) => setTipoOcasional(e.target.value)}>
                <option value="">Seleccione el tipo de docente ocasional</option>
              ...
              <button className="info-button" onClick={toggleValoresTipo}>Info</button>
            </div>
            {mostrarValoresTipo && (
            ...
            <div className="cualificacion-container">
              <select value={cualificacion} onChange={(e) => setCualificacion(e.target.value as 'Especialización' | 'Maestría' | 'Doctorado' | 'Ninguna')}>
                <option value="Ninguna">Ninguna</option>
                <option value="Especialización">Especialización</option>
                <option value="Maestría">Maestría</option>
                <option value="Doctorado">Doctorado</option>
              </select>
              <button className="info-button" onClick={toggleValoresCualificacion}>Info</button>
            </div>
            ...
            <div className="grupo-colciencias-container">
              <select value={grupoColciencias} onChange={(e) => setGrupoColciencias(e.target.value as 'Grupo A1' | 'Grupo A' | 'Grupo B' | 'Grupo C' | 'Grupos Reconocidos' | 'Semillero' | 'Ninguno')}>
              ...
              <button className="info-button" onClick={toggleValoresGrupo}>Info</button>
            </div>
            {mostrarValoresGrupo && (
              <div className="valores-grupo-colciencias">
                <h3>Valores del grupo Colciencias</h3>
                <ul>
                  {Object.entries(grupoColcienciasValores).map(([key, value]) => (
                    <li key={key}>
                      {key}: {value}
                    </li>
                  ))}
            ...
            <button className="add-button" onClick={handleAddTeacher}>Agregar Docente</button>
            <input type="text" placeholder="ID a borrar" value={idBorrar} onChange={(e) => setIdBorrar(e.target.value)} />
            <button className="delete-button" onClick={handleDeleteTeacher}>Borrar Docente</button>
          </div>
        </div>
        <div className="tabs">
          <button className={`tab-button ${activeTab === 'docentes' ? 'active' : ''}`} onClick={toggleTeacherList}>Lista de Docentes</button>
          <button className={`tab-button ${activeTab === 'salarios' ? 'active' : ''}`} onClick={handleShowNotification}>Total Salarios</button>
        </div>
